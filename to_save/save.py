import os, configparser, subprocess, hashlib, argparse, sqlite3
"""
...
def get_info():
	...
	for f in os.listdir(tosavepath):
		output = subprocess.run(["%s"%(trhpath), "%s/%s"%(tosavepath,f)], capture_output=True)

		if output.stdout.decode("utf-8") != '':
			data_dic = {}
			data_dic["trh_hash"] = output.stdout.decode("utf-8").rstrip()
			data_dic["sha256"] = hashlib.sha256(open('%s/%s'%(tosavepath,f),'rb').read()).hexdigest()
...
def save_data(conn, d):
	...
	cur = conn.cursor()
	try:
		cur.execute("INSERT INTO data(sha256,detection,typeRefHash) VALUES (?, ?, ?)", (d["sha256"], d["detection"].lower(), d["trh_hash"]))
	except Error as e:
		print(e)
	return cur
